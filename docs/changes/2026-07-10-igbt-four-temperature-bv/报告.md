...
- 本轮未启用 HeavyIon，不能作为 SEB 结果；现有曲线、最终 TDR 和场分布仅用于关断态 BV 基线与后续模型冻结。

## 结构与运行配置

- 总厚度：451.15 µm。
- 净层序：n+ 0.65 µm、p-body 5 µm、n-drift 440 µm、n-buffer 5 µm、p+ collector 0.5 µm。
- n-drift：5.0e12 cm^-3。
...
- 最终远端目录：`/home/tcad/codex_runs/igbt_bv_first_round_final_20260711_090323`。
...
- n+/p-body：约 0.6584 µm；目标 0.65 µm。
...
- **不满足“高场和最大电流密度都远离栅氧”的验收条件。** 243.15 K 与 298.15 K 的最大电场落在 Si/栅氧界面；四个温度的最大总电流密度均位于距该界面约 0.0015 µm 的首层硅网格点。
...
- 网格：artifacts/igbt_bv_msh.tdr、artifacts/igbt_bv_msh.log。
- 曲线：artifacts/Icollector-Vce_*.plt、根目录同名 CSV。
- 最终场分布：artifacts/bv_*K.tdr。
- 四温度 × 四物理量图片：artifacts/*K_electric_field.png、*K_total_current_density.png、*K_temperature.png、*K_avalanche_generation.png。
