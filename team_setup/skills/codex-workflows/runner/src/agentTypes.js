...
// reading a Codex agent definition. Project .codex/agents takes precedence over
// ~/.codex/agents; .claude/agents remains a compatibility fallback.
...
export async function loadAgentType(name, cwd = process.cwd()) {
  ...
  const rels = [join(".codex", "agents", `${name}.md`), join(".claude", "agents", `${name}.md`)];
  let found = null;
  for (const rel of rels) {
    found = await findUp(cwd, rel);
    if (found) break;
  }
  found ??= await tryRead(join(homedir(), ".codex", "agents", `${name}.md`));
  found ??= await tryRead(join(homedir(), ".claude", "agents", `${name}.md`));
