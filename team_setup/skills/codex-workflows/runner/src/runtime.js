...
// scope shadowing home. Prefer Codex locations; keep Claude locations as fallback.
// `<name>.js` and `<name>.workflow.js` both accepted.
function resolveNamedWorkflow(name) {
  const dirs = [
    join(process.cwd(), ".codex", "workflows"),
    join(process.cwd(), ".claude", "workflows"),
    join(os.homedir(), ".codex", "workflows"),
