...
// 9) agentType: read system prompt + model from .codex/agents/<name>.md.
{
  ...
  await writeFile(
    join(root, ".codex", "agents", "terse.md"),
...
// 19) workflow("name") resolves a saved workflow from .codex/workflows/.
{
  const root = await mkdtemp(join(tmpdir(), "wf-registry-"));
  await mkdir(join(root, ".codex", "workflows"), { recursive: true });
  await writeFile(join(root, ".codex", "workflows", "child.js"), 'export const meta = { name: "child" };\nreturn 7;\n');
  const prev = process.cwd();
  process.chdir(root);
  try {
    const r = await runWorkflowSource('export const meta = { name: "parent" };\nreturn await workflow("child");', {});
    assert.equal(r, 7, "named workflow resolved from .codex/workflows and ran");
  } finally {
    process.chdir(prev);
    await rm(root, { recursive: true, force: true });
  }
